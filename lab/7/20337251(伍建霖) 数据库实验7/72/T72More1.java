...
public class T72More1 {
    ...
    public static void copyTableByJDBC(String src_JDBC_DRIVER, String src_DB_URL, String src_USER, String src_PASS,
        ...
        try {
            ...
            // step 2 连接数据库 ：Connection con = DriverManager.getConnection(dbURL, userName,
            // userPwd);
            ...
            target_ps = target_conn.prepareStatement(
                    "create table T72_department   (dept_name  varchar(20) not null, building varchar(15), budget  numeric(12,2))");
            target_ps.executeUpdate();

            target_ps = target_conn
                    .prepareStatement("INSERT INTO T72_department (dept_name, building, budget) VALUES(?, ?, ?)");
            ...
            while (src_rs.next()) {
                ...
                String dept_building = src_rs.getString("building");
                Float dept_budget = src_rs.getFloat("budget");
                // 输出数据
                System.out.print(dept_name);
                System.out.print("\t" + dept_building);
                System.out.print("\t" + dept_budget);
                System.out.print("\n");

                // step 4 给占位符赋值
                target_ps.setString(1, dept_name);// 给第3个占位符赋值(dept_name)
                target_ps.setString(2, dept_building); // 给第1个占位符赋值(ID)
                target_ps.setFloat(3, dept_budget); // 给第4个占位符赋值(salary)
